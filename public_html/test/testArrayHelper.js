require(['tools/data/ArrayHelper'], function(ArrayHelper){
	...
	QUnit.test('intersection - should keep only items that are present on all arrays', function(){
		var result = ArrayHelper.intersection(['0', -1, 1], [1, 2, 3], [1, 10, 11]);

		deepEqual(result, [1]);
	});

	QUnit.test('intersection - should remove duplicates', function(){
		var result = ArrayHelper.intersection([-1, 2, 1], [0, -1, 1]);

		deepEqual(result, [-1, 1]);
	});

	QUnit.test('intersection - should return an empty array if no intersection', function(){
		var result = ArrayHelper.intersection([0], [1, 2]);

		deepEqual(result, []);
	});

	QUnit.test('intersection - should use empty array when null/undefined', function(){
		deepEqual(ArrayHelper.intersection([1, 2], null), []);
		deepEqual(ArrayHelper.intersection([1, 2], undefined), []);
	});

	QUnit.test('unique - should remove duplicates', function(){
		var result = ArrayHelper.unique(['a', 1, 2, 'c', 'b', 2, 1, 'b', 'c']);

		deepEqual(result, ['a', 1, 2, 'c', 'b']);
	});

	QUnit.test('unique - should merge multiple inputs', function(){
		var result = ArrayHelper.unique(['a', 1, 2, 'c', 'b'], [2, 1, 'b', 'd'], ['a']);

		deepEqual(result, ['a', 1, 2, 'c', 'b', 'd']);
	});

	QUnit.test('unique - should return null/undefined array if source array is null/undefined', function(){
		deepEqual(ArrayHelper.unique(null), []);
		deepEqual(ArrayHelper.unique(undefined), []);
	});

	QUnit.test('difference - should keep only items that are present on 1st array but not present on other arrays', function(){
		var a = ['a', 'b', 1],
			b = ['c', 1],
			c = [1, 2, 3];
	...
	QUnit.test('difference - should remove duplicates', function(){
		var a = ['a', 'b', 1, 'b'],
			b = ['c', 'a', 1],
			c = [1, 2, 3, 'a'];
	...
	QUnit.test('difference - should return an empty array if items are present on other arrays', function(){
		var a = ['b', 'a', 'c', 1, 2, 3],
			b = ['c', 'a'],
			c = [1, 'b', 2, 3, 'a'];
	...
	QUnit.test('binaryIndexOf - should not find the element', function(){
		var arr = [1, 2, 5, 7, 12, 19, 31];
		equal(ArrayHelper.binaryIndexOf(arr, 8), -4);
	});

	QUnit.test('partition', function(){
