require(['tools/math/Fraction'], function(Fraction){
	QUnit.module('Fraction');

	QUnit.test('constructor with object', function(){
		var f = new Fraction({
			num: -13,
			den: 27
		});

		equal(f.num, 13);
		equal(f.den, 27);
		equal(f.sgn, -1);
	});

	QUnit.test('constructor with array', function(){
		var f = new Fraction(-13, 27);

		equal(f.num, 13);
		equal(f.den, 27);
		equal(f.sgn, -1);
	});

	QUnit.test('constructor with array, reduced', function(){
		var f = new Fraction(36, 12);

		equal(f.num, 3);
		equal(f.den, 1);
		equal(f.sgn, 1);
	});

	QUnit.test('constructor with number', function(){
		var f = new Fraction(-13.27);

		equal(f.num, 1327);
		equal(f.den, 100);
		equal(f.sgn, -1);
	});

	QUnit.test('constructor with big number', function(){
		var f = Fraction.fromNumber(1234567.9991829, 1e-10);

		equal(f.num, 385102500000);
		equal(f.den, 311933);
	});

	QUnit.test('constructor with long number', function(){
		var f = Fraction.fromNumber(0.9999999999);

		equal(f.num, 9999999999);
		equal(f.den, 10000000000);
	});

	QUnit.test('constructor with big number as string', function(){
		var f = new Fraction('1234567.9991829');

		equal(f.num, 12345679991829);
		equal(f.den, 10000000);
	});

	QUnit.test('constructor with number', function(){
		var f = Fraction.fromNumber(355 / 113);

		equal(f.num, 355);
		equal(f.den, 113);
		equal(f.sgn, 1);
	});

	QUnit.test('constructor with string (negative)', function(){
		var f = new Fraction('-13.27');

		equal(f.num, 1327);
		equal(f.den, 100);
		equal(f.sgn, -1);
	});

	QUnit.test('constructor with string (positive)', function(){
		var f = new Fraction('5.12');

		equal(f.num, 128);
		equal(f.den, 25);
		equal(f.sgn, 1);
	});

	QUnit.test('constructor with string (repeating decimals)', function(){
		var f = new Fraction('5.13(31)');

		equal(f.num, 25409);
		equal(f.den, 4950);
		equal(f.sgn, 1);
	});

	QUnit.test('constructor with string fraction', function(){
		var f = new Fraction('5/13');

		equal(f.num, 5);
		equal(f.den, 13);
		equal(f.sgn, 1);
	});

	QUnit.test('constructor with positive infinite', function(){
		var f = new Fraction(10, 0);

		equal(f.num, 1);
		equal(f.den, 0);
		equal(f.sgn, 1);
	});

	QUnit.test('constructor with negative infinite', function(){
		var f = new Fraction(-10, 0);

		equal(f.num, 1);
		equal(f.den, 0);
		equal(f.sgn, -1);
	});

	QUnit.test('GCD', function(){
		var f1 = new Fraction('5/8');
		var f2 = new Fraction('3/7');

		equal(f1.gcd(f2).toString(), new Fraction('1/56').toString());
	});

	QUnit.test('add', function(){
		var f1 = new Fraction('5.13(31)');
		var f2 = new Fraction('2.3(7)');

		equal(f1.add(f2).toNumber(), new Fraction('7.51(09)').toNumber());
	});

	QUnit.test('mul', function(){
		var f1 = new Fraction('5.13(31)');
		var f2 = new Fraction('2.3(7)');

		equal(f1.mul(f2).toNumber(), new Fraction(2718763, 222750).toNumber());
	});

	QUnit.test('abs with positive number', function(){
		var f = new Fraction(1, 7);

		equal(f.abs().toNumber(), new Fraction(1, 7).toNumber());
	});

	QUnit.test('abs with negative number', function(){
		var f = new Fraction(-1, 7);

		equal(f.abs().toNumber(), new Fraction(1, 7).toNumber());
	});

	QUnit.test('pow with positive number', function(){
		var f = new Fraction(2, 3);

		equal(f.pow(2).toNumber(), new Fraction(4, 9).toNumber());
	});

	QUnit.test('pow with negative number', function(){
		var f = new Fraction(-2, 3);

		equal(f.pow(-2).toNumber(), new Fraction(9, 4).toNumber());
	});

	QUnit.test('div', function(){
		var f1 = new Fraction('123.32');
		var f2 = new Fraction('33.6(567)');

		equal(f1.div(f2).toString(), '3.6()');
	});

	QUnit.test('mod', function(){
		var f1 = new Fraction(4.55);
		var f2 = new Fraction(0.05);

		equal(f1.mod(f2).toNumber(), new Fraction(0).toNumber());
	});

	QUnit.test('isDivisible true', function(){
		var f1 = new Fraction(19, 6);
		var f2 = new Fraction(38, 12);

		ok(f1.isDivisibleBy(f2));
	});

	QUnit.test('isDivisible false', function(){
		var f1 = new Fraction(19, 6);
		var f2 = new Fraction(5, 98);

		notOk(f1.isDivisibleBy(f2));
	});

	QUnit.test('to positive string 1', function(){
		var f = new Fraction('5.13(31)');

		equal(f.toString(), '5.13(31)');
	});

	QUnit.test('to positive string 2', function(){
		var f = new Fraction('0.13(31)');

		equal(f.toString(), '0.13(31)');
	});

	QUnit.test('to positive string with zeros 1', function(){
		var f = new Fraction('5000.13(31)');

		equal(f.toString(), '5000.13(31)');
	});

	QUnit.test('to positive string with zeros 2', function(){
		var f = new Fraction('5.00013(31)');

		equal(f.toString(), '5.00013(31)');
	});

	QUnit.test('to negative string', function(){
		var f = new Fraction('-5.13(31)');

		equal(f.toString(), '-5.13(31)');
	});

	QUnit.test('toString', function(){
		var f = new Fraction('1.2');

		equal(f.toString(), '1.2');
	});

	QUnit.test('toString +Infinity', function(){
		var f = new Fraction('1/0');

		equal(f.toString(), '+Inf');
	});

	QUnit.test('toString -Infinity', function(){
		var f = new Fraction('-1/0');

		equal(f.toString(), '-Inf');
	});

	QUnit.test('toString NaN', function(){
		var f = new Fraction('0/0');

		equal(f.toString(), 'NaN');
	});

	QUnit.test('to LaTeX integer', function(){
		var f = new Fraction('-10');

		equal(f.toLaTeX(), '-10');
	});

	QUnit.test('to LaTeX positive fraction', function(){
		var f = new Fraction('1.5');

		equal(f.toLaTeX(), '\\frac{3}{2}');
	});

	QUnit.test('to LaTeX negative fraction', function(){
		var f = new Fraction('-1.5');

		equal(f.toLaTeX(), '-\\frac{3}{2}');
	});
});
