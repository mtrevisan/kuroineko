 ...
 */
define(['tools/data/ArrayHelper', 'tools/data/structs/Trie'], function(ArrayHelper, Trie){
	...
	var hypenate = function(word){
		...
		if(word.indexOf(softHyphen) >= 0)
			...
			var size = ww.length,
				...
				i, j;
			for(i = 0; i < size; i ++){
				var res = trie.contains(ww.substring(i));
				if(res){
					j = 0;
					trieData[ww.substring(i, res.index + 2)].split('').forEach(function(d, idx){
						d = parseInt(d);
						if(isNaN(d))
							j ++;
			...
			hypenatedWord = word.split('').map(function(chr, idx){
				return (idx >= leftmin && idx <= maxLength && this[idx + 1] % 2? softHyphen: '') + chr;
			}, hyp).join('');
